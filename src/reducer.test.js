...
describe('reducer', () => {
  ...
  describe('fetch된 Region 데이터가 변경되었을 때', () => {
    it('변경된 region state가 반환된다.', () => {
      const state = reducer(prevState, setRegions({ regions: [{ id: 1, name: '서울' }] }));
      expect(state.regions[0].name).toBe('서울');
    });
  });
  describe('fetch된 Category 데이터가 변경되었을 때', () => {
    it('변경된 region state가 반환된다.', () => {
      const state = reducer(prevState, setCategories({ categories: [{ id: 1, name: '한식' }] }));
      expect(state.categories[0].name).toBe('한식');
    });
  });
  describe('fetch된 Restaurant 데이터가 변경되었을 때', () => {
    it('변경된 region state가 반환된다.', () => {
      const state = reducer(prevState, setRestaurants({ restaurants: [{ id: 1, name: '함지박식당' }] }));
      expect(state.restaurants[0].name).toBe('함지박식당');
    });
  });
